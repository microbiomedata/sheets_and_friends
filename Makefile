...
# clean? cogs_fetch?
artifacts/slots_modified.yaml: .cogs/tracked/modifications_long.tsv artifacts/with_shuttles.yaml
	poetry run mod_by_path \
		--config_tsv $< \
		--yaml_input $(word 2,$^) \
		--yaml_output $@  2>> logs/mod_by_path.log

artifacts/nmdc_dh.yaml: .cogs/tracked/validation_converter.tsv artifacts/slots_modified.yaml
	poetry run add_regex_validators \
		--config_tsv $< \
		--yaml_input $(word 2,$^) \
		--yaml_output $@  2>> logs/regex_validation.log
